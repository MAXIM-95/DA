...
enum
{
    NPREF	= 1, //кол-во слов в префиксе
...
//функция поиска префикса по таблице (или создаёт его по запрсу).
//возвращает указатель на префикс или NULL в случае неудачи.
State	*search(char *prefix[], int create);
//считывает входные данные и строит таблицу префиксов.
void	build_table(char *prefix[], FILE*);
//генерация текста
void	text_gen(unsigned int nwords, char* startWord);
//поплнение списка суффиксов и обновление префикса
void	add_pref(char *prefix[], char *word);
//добавление нового суффикса в список
